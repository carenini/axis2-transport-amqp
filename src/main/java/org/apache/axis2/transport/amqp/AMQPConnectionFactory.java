...
public class AMQPConnectionFactory {
    ...
    /** The shared JMS connection for this JMS connection factory */
    private int cacheLevel = AMQPConstants.CACHE_CONNECTION;
    ...
    public AMQPConnectionFactory(Parameter parameter) {
        ...
        try {
            ...
            conFactory = AMQPUtils.lookup(context, ConnectionFactory.class,
                parameters.get(AMQPConstants.PARAM_CONFAC_JNDI_NAME));
            if (parameters.get(AMQPConstants.PARAM_DESTINATION) != null) {
                sharedDestination = AMQPUtils.lookup(context, Destination.class,
                    parameters.get(AMQPConstants.PARAM_DESTINATION));
            }
            log.info("JMS ConnectionFactory : " + name + " initialized");

        } catch (NamingException e) {
            throw new AxisAMQPException("Cannot acquire JNDI context, JMS Connection factory : " +
                parameters.get(AMQPConstants.PARAM_CONFAC_JNDI_NAME) + " or default destination : " +
                parameters.get(AMQPConstants.PARAM_DESTINATION) +
    ...
    private void digestCacheLevel() {
        ...
        if ("none".equalsIgnoreCase(val)) {
            this.cacheLevel = AMQPConstants.CACHE_NONE;
        } else if ("connection".equalsIgnoreCase(val)) {
            this.cacheLevel = AMQPConstants.CACHE_CONNECTION;
        } else if ("session".equals(val)){
            this.cacheLevel = AMQPConstants.CACHE_SESSION;
        } else if ("producer".equals(val)) {
            this.cacheLevel = AMQPConstants.CACHE_PRODUCER;
        } else if (val != null) {
            throw new AxisAMQPException("Invalid cache level : " + val + " for JMS CF : " + name);
        }
    }
    ...
    public Destination getDestination(String destinationName, String destinationType) {
        try {
            return AMQPUtils.lookupDestination(context, destinationName, destinationType);
        } catch (NamingException e) {
    ...
    public String getReplyToDestination() {
        return parameters.get(AMQPConstants.PARAM_REPLY_DESTINATION);
    }
    ...
    public String getReplyDestinationType() {
        return parameters.get(AMQPConstants.PARAM_REPLY_DEST_TYPE) != null ?
                parameters.get(AMQPConstants.PARAM_REPLY_DEST_TYPE) :
                AMQPConstants.DESTINATION_TYPE_GENERIC;
    }
    ...
    public boolean isJmsSpec11() {
        return parameters.get(AMQPConstants.PARAM_JMS_SPEC_VER) == null ||
            "1.1".equals(parameters.get(AMQPConstants.PARAM_JMS_SPEC_VER));
    }
    ...
    public Boolean isQueue() {
        if (parameters.get(AMQPConstants.PARAM_CONFAC_TYPE) == null &&
            parameters.get(AMQPConstants.PARAM_DEST_TYPE) == null) {
            return null;
        }

        if (parameters.get(AMQPConstants.PARAM_CONFAC_TYPE) != null) {
            if ("queue".equalsIgnoreCase(parameters.get(AMQPConstants.PARAM_CONFAC_TYPE))) {
                return true;
            } else if ("topic".equalsIgnoreCase(parameters.get(AMQPConstants.PARAM_CONFAC_TYPE))) {
                return false;
            } else {
                throw new AxisAMQPException("Invalid " + AMQPConstants.PARAM_CONFAC_TYPE + " : " +
                    parameters.get(AMQPConstants.PARAM_CONFAC_TYPE) + " for JMS CF : " + name);
            }
        } else {
            if ("queue".equalsIgnoreCase(parameters.get(AMQPConstants.PARAM_DEST_TYPE))) {
                return true;
            } else if ("topic".equalsIgnoreCase(parameters.get(AMQPConstants.PARAM_DEST_TYPE))) {
                return false;
            } else {
                throw new AxisAMQPException("Invalid " + AMQPConstants.PARAM_DEST_TYPE + " : " +
                    parameters.get(AMQPConstants.PARAM_DEST_TYPE) + " for JMS CF : " + name);
            }
        }
    }
    ...
    private boolean isSessionTransacted() {
        return parameters.get(AMQPConstants.PARAM_SESSION_TRANSACTED) == null ||
            Boolean.valueOf(parameters.get(AMQPConstants.PARAM_SESSION_TRANSACTED));
    }
    ...
    private Connection createConnection() {
        ...
        try {
            connection = AMQPUtils.createConnection(
                conFactory,
                parameters.get(AMQPConstants.PARAM_JMS_USERNAME),
                parameters.get(AMQPConstants.PARAM_JMS_PASSWORD),
                isJmsSpec11(), isQueue());
    ...
    private Session createSession(Connection connection) {
        try {
            ...
            return AMQPUtils.createSession(
    ...
    private MessageProducer createProducer(Session session, Destination destination) {
        try {
            ...
            return AMQPUtils.createProducer(
    ...
    public Connection getConnection() {
        if (cacheLevel > AMQPConstants.CACHE_NONE) {
    ...
    public MessageProducer getMessageProducer(
        ...
        if (cacheLevel > AMQPConstants.CACHE_SESSION) {
