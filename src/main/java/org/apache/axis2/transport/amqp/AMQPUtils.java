...
public class AMQPUtils extends BaseUtils {
    ...
    public static String getDestination(String url) {
        String tempUrl = url.substring(AMQPConstants.JMS_PREFIX.length());
    ...
    public static void setSOAPEnvelope(Message message, MessageContext msgContext, String contentType)
        throws AxisFault, AmqpException {
     ...
     * @throws AmqpException on exception
     */
    public static void setTransportHeaders(MessageContext msgContext, Message message)
        ...
        for (Object headerName : headerMap.keySet()) {
            ...
            if (name.startsWith(AMQPConstants.JMSX_PREFIX) &&
                !(name.equals(AMQPConstants.JMSX_GROUP_ID) || name.equals(AMQPConstants.JMSX_GROUP_SEQ))) {
                continue;
            }

            if (AMQPConstants.JMS_COORELATION_ID.equals(name)) {
                message.setJMSCorrelationID(
                        (String) headerMap.get(AMQPConstants.JMS_COORELATION_ID));
            } else if (AMQPConstants.JMS_DELIVERY_MODE.equals(name)) {
                Object o = headerMap.get(AMQPConstants.JMS_DELIVERY_MODE);
            ...
            } else if (AMQPConstants.JMS_EXPIRATION.equals(name)) {
                message.setJMSExpiration(
                    Long.parseLong((String) headerMap.get(AMQPConstants.JMS_EXPIRATION)));
            } else if (AMQPConstants.JMS_MESSAGE_ID.equals(name)) {
                message.setJMSMessageID((String) headerMap.get(AMQPConstants.JMS_MESSAGE_ID));
            } else if (AMQPConstants.JMS_PRIORITY.equals(name)) {
                message.setJMSPriority(
                    Integer.parseInt((String) headerMap.get(AMQPConstants.JMS_PRIORITY)));
            } else if (AMQPConstants.JMS_TIMESTAMP.equals(name)) {
                message.setJMSTimestamp(
                    Long.parseLong((String) headerMap.get(AMQPConstants.JMS_TIMESTAMP)));
            } else if (AMQPConstants.JMS_MESSAGE_TYPE.equals(name)) {
                message.setJMSType((String) headerMap.get(AMQPConstants.JMS_MESSAGE_TYPE));
    ...
    public static Map<String, Object> getTransportHeaders(Message message) {
        ...
        try {
            if (message.getJMSCorrelationID() != null) {
                map.put(AMQPConstants.JMS_COORELATION_ID, message.getJMSCorrelationID());
            }
        } catch (AmqpException ignore) {}

        // set the delivery mode as persistent or not
        try {
            map.put(AMQPConstants.JMS_DELIVERY_MODE, Integer.toString(message.getJMSDeliveryMode()));
        } catch (AmqpException ignore) {}
        ...
        try {
            if (message.getJMSDestination() != null) {
                ...
                map.put(AMQPConstants.JMS_DESTINATION,
        ...
        // expiration
        try {
            map.put(AMQPConstants.JMS_EXPIRATION, Long.toString(message.getJMSExpiration()));
        } catch (AmqpException ignore) {}

        // if a JMS message ID is found
        try {
            if (message.getJMSMessageID() != null) {
                map.put(AMQPConstants.JMS_MESSAGE_ID, message.getJMSMessageID());
            }
        } catch (AmqpException ignore) {}

        // priority
        try {
            map.put(AMQPConstants.JMS_PRIORITY, Long.toString(message.getJMSPriority()));
        } catch (AmqpException ignore) {}

        // redelivered
        try {
            map.put(AMQPConstants.JMS_REDELIVERED, Boolean.toString(message.getJMSRedelivered()));
        } catch (AmqpException ignore) {}
        ...
        try {
            if (message.getJMSReplyTo() != null) {
                ...
                map.put(AMQPConstants.JMS_REPLY_TO,
                    dest instanceof Queue ?
                        ((Queue) dest).getQueueName() : ((Topic) dest).getTopicName());
            }
        } catch (AmqpException ignore) {}
        ...
        // priority
        try {
            map.put(AMQPConstants.JMS_TIMESTAMP, Long.toString(message.getJMSTimestamp()));
        } catch (AmqpException ignore) {}

        // message type
        try {
            if (message.getJMSType() != null) {
                map.put(AMQPConstants.JMS_TYPE, message.getJMSType());
            }
        } catch (AmqpException ignore) {}
     ...
     * @throws AmqpException
     */
    public static Destination createTemporaryDestination(Session session) throws AmqpException {
     ...
     * @throws AmqpException
     */
    public static long getMessageSize(Message message) throws AmqpException {
        if (message instanceof BytesMessage) {
            return AMQPUtils.getBodyLength((BytesMessage) message);
        } else if (message instanceof TextMessage) {
     ...
     * @throws AmqpException on errors, to be handled and logged by the caller
     */
    public static Connection createConnection(ConnectionFactory conFac,
        String user, String pass, boolean jmsSpec11, Boolean isQueue) throws AmqpException {
    ...
    public static Session createSession(Connection connection, boolean transacted, int ackMode,
        boolean jmsSpec11, Boolean isQueue) throws AmqpException {
    ...
    public static MessageConsumer createConsumer(
        ...
        boolean isDurable, boolean jmsSpec11) throws AmqpException {
    ...
    public static MessageProducer createProducer(
        Session session, Destination destination, Boolean isQueue, boolean jmsSpec11) throws AmqpException {
    ...
    public static String getDestinationTypeAsString(int destType) {
        if (destType == AMQPConstants.QUEUE) {
            return "Queue";
        } else if (destType == AMQPConstants.TOPIC) {
            return "Topic";
        } else {
            return "Generic";
        }
    ...
    public static Destination lookupDestination(Context context, String destinationName,
        ...
        try {
            return AMQPUtils.lookup(context, Destination.class, destinationName);
        } catch (NameNotFoundException e) {
            try {
                return AMQPUtils.lookup(context, Destination.class,
                    (AMQPConstants.DESTINATION_TYPE_TOPIC.equals(destinationType) ?
