...
public class AMQPSender extends AbstractTransportSender implements ManagementSupport {
     ...
     */
    private AMQPConnectionFactory getJMSConnectionFactory(AMQPOutTransportInfo trpInfo) {
        Map<String,String> props = trpInfo.getProperties();
        if (trpInfo.getProperties() != null) {
            String jmsConnectionFactoryName = props.get(AMQPConstants.PARAM_JMS_CONFAC);
    ...
    public void sendMessage(MessageContext msgCtx, String targetAddress,
        ...
        AMQPConnectionFactory jmsConnectionFactory = null;
        AMQPOutTransportInfo jmsOut = null;
        AMQPMessageSender messageSender = null;

        if (targetAddress != null) {

            jmsOut = new AMQPOutTransportInfo(targetAddress);
            ...
            if (jmsConnectionFactory != null) {
                messageSender = new AMQPMessageSender(jmsConnectionFactory, targetAddress);
            ...
            jmsOut = (AMQPOutTransportInfo) outTransportInfo;
            try {
                messageSender = jmsOut.createJMSSender();
            } catch (JMSException e) {
                handleException("Unable to create a JMSMessageSender for : " + outTransportInfo, e);
        ...
        String contentTypeProperty =
            (String) msgCtx.getProperty(AMQPConstants.CONTENT_TYPE_PROPERTY_PARAM);
     ...
     */
    private void sendOverJMS(MessageContext msgCtx, AMQPMessageSender messageSender,
        String contentTypeProperty, AMQPConnectionFactory jmsConnectionFactory,
        AMQPOutTransportInfo jmsOut) throws AxisFault {
        ...
        if (waitForResponse) {
            ...
            String replyDestName = (String) msgCtx.getProperty(AMQPConstants.JMS_REPLY_TO);
            if (replyDestName == null && jmsConnectionFactory != null) {
                replyDestName = jmsConnectionFactory.getReplyToDestination();
            }

            String replyDestType = (String) msgCtx.getProperty(AMQPConstants.JMS_REPLY_TO_TYPE);
            ...
            replyDestination = AMQPUtils.setReplyDestination(
        ...
        try {
            metrics.incrementBytesSent(msgCtx, AMQPUtils.getMessageSize(message));
        } catch (JMSException e) {
    ...
    private void waitForResponseAndProcess(Session session, Destination replyDestination,
        ...
        try {
            ...
            consumer = AMQPUtils.createConsumer(session, replyDestination,
                "JMSCorrelationID = '" + correlationId + "'");

            // how long are we willing to wait for the sync response
            long timeout = AMQPConstants.DEFAULT_JMS_TIMEOUT;
            String waitReply = (String) msgCtx.getProperty(AMQPConstants.JMS_WAIT_REPLY);
            if (waitReply != null) {
            ...
            if (reply != null) {
                ...
                try {
                    metrics.incrementBytesReceived(AMQPUtils.getMessageSize(reply));
                } catch (JMSException e) {
                    log.warn("Error reading JMS message size to update transport metrics", e);
                }
    ...
    private Message createJMSMessage(MessageContext msgContext, Session session,
        ...
        Message message = null;
        String msgType = getProperty(msgContext, AMQPConstants.JMS_MESSAGE_TYPE);
        ...
        if (jmsPayloadType == null) {
            ...
            boolean useBytesMessage =
                msgType != null && AMQPConstants.JMS_BYTE_MESSAGE.equals(msgType) ||
        ...
        } else if (AMQPConstants.JMS_BYTE_MESSAGE.equals(jmsPayloadType)) {
        ...
        } else if (AMQPConstants.JMS_TEXT_MESSAGE.equals(jmsPayloadType)) {
        ...
        String correlationId = getProperty(msgContext, AMQPConstants.JMS_COORELATION_ID);
    ...
    private String guessMessageType(MessageContext msgContext) {
        ...
        if (firstChild != null) {
            if (BaseConstants.DEFAULT_BINARY_WRAPPER.equals(firstChild.getQName())) {
                return AMQPConstants.JMS_BYTE_MESSAGE;
            } else if (BaseConstants.DEFAULT_TEXT_WRAPPER.equals(firstChild.getQName())) {
                return AMQPConstants.JMS_TEXT_MESSAGE;
            }
        }
        return null;
    }
    ...
    private void processSyncResponse(MessageContext outMsgCtx, Message message,
        ...
        handleIncomingMessage(
            ...
            AMQPUtils.getProperty(message, BaseConstants.SOAPACTION),
            contentType
        );
    }
