...
public class AMQPOutTransportInfo implements OutTransportInfo {
     ...
     */
    private String destinationType = AMQPConstants.DESTINATION_TYPE_GENERIC;
     ...
     */
    AMQPOutTransportInfo(AMQPConnectionFactory jmsConnectionFactory, Destination dest,
            String contentTypeProperty) {
        this.jmsConnectionFactory = jmsConnectionFactory;
        this.destination = dest;
        destinationType = dest instanceof Topic ? AMQPConstants.DESTINATION_TYPE_TOPIC
                                                : AMQPConstants.DESTINATION_TYPE_QUEUE;
     ...
     */
    AMQPOutTransportInfo(String targetEPR) {

        this.targetEPR = targetEPR;
        if (!targetEPR.startsWith(AMQPConstants.JMS_PREFIX)) {
    ...
    private Destination getDestination(Context context, String url) {
        String destinationName = AMQPUtils.getDestination(url);
    ...
    private Destination getReplyDestination(Context context, String url) {
        String replyDestinationName = properties.get(AMQPConstants.PARAM_REPLY_DESTINATION);
        if (log.isDebugEnabled()) {
        ...
        try {
            return AMQPUtils.lookupDestination(context, replyDestinationName, replyDestinationType);
        } catch (NamingException e) {
    ...
    public Destination getReplyDestination(String replyDest) {
        ...
        try {
            return AMQPUtils.lookupDestination(
     ...
     */
    public AMQPMessageSender createJMSSender() throws JMSException {
        ...
        } else if (AMQPConstants.DESTINATION_TYPE_TOPIC.equals(destinationType)) {
            destType = AMQPConstants.TOPIC;
        ...
        return new AMQPMessageSender(
                ...
                jmsConnectionFactory == null ?
                        AMQPConstants.CACHE_NONE : jmsConnectionFactory.getCacheLevel(),
                false,
                destType == -1 ?
                        null : destType == AMQPConstants.QUEUE ? Boolean.TRUE : Boolean.FALSE
        );
