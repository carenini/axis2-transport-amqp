...
public class AMQPConnectionFactoryManager {
    ...
    private final Map<String,AMQPConnectionFactory> connectionFactories =
        new HashMap<String,AMQPConnectionFactory>();
    ...
    private void loadConnectionFactoryDefinitions(ParameterInclude trpDesc) {
        ...
        for (Parameter p : trpDesc.getParameters()) {
            try {
                AMQPConnectionFactory jmsConFactory = new AMQPConnectionFactory(p);
     ...
     */
    public AMQPConnectionFactory getJMSConnectionFactory(String name) {
     ...
     */
    public AMQPConnectionFactory getJMSConnectionFactory(Map<String,String> props) {
        for (AMQPConnectionFactory cf : connectionFactories.values()) {
            Map<String,String> cfProperties = cf.getParameters();

            if (equals(props.get(AMQPConstants.PARAM_CONFAC_JNDI_NAME),
                cfProperties.get(AMQPConstants.PARAM_CONFAC_JNDI_NAME))
                &&
	...
	public AMQPConnectionFactory getAMQPConnectionFactory(String value) {
