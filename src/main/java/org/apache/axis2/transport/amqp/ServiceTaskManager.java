...
public class ServiceTaskManager {
    ...
    private class MessageListenerTask implements Runnable, ExceptionListener {
        ...
        private void closeConnection() {
            if (connection != null &&
                cacheLevel < AMQPConstants.CACHE_CONNECTION) {
                try {
                    if (log.isDebugEnabled()) {
        ...
        private void closeSession(boolean forced) {
            if (session != null &&
                (cacheLevel < AMQPConstants.CACHE_SESSION || forced)) {
                try {
                    if (log.isDebugEnabled()) {
        ...
        private void closeConsumer(boolean forced) {
            if (consumer != null &&
                (cacheLevel < AMQPConstants.CACHE_CONSUMER || forced)) {
                try {
                    if (log.isDebugEnabled()) {
        ...
        private Connection createConnection() {
            ...
            try {
                connection = AMQPUtils.createConnection(
                    conFactory,
                    jmsProperties.get(AMQPConstants.PARAM_JMS_USERNAME),
                    jmsProperties.get(AMQPConstants.PARAM_JMS_PASSWORD),
                    isJmsSpec11(), isQueue());
        ...
        private MessageConsumer createConsumer() {
            try {
                ...
                return AMQPUtils.createConsumer(
    ...
    private Destination getDestination(Session session) {
        if (destination == null) {
            try {
                ...
                destination = AMQPUtils.lookupDestination(context, getDestinationJNDIName(),
                        AMQPUtils.getDestinationTypeAsString(destinationType));
                if (log.isDebugEnabled()) {
    ...
    private UserTransaction getUserTransaction() {
        if (!cacheUserTransaction) {
            ...
            try {
                context = getInitialContext();
                return
                    AMQPUtils.lookup(context, UserTransaction.class, getUserTransactionJNDIName());
            } catch (NamingException e) {
    ...
    public void setJmsMessageReceiver(AMQPMessageReceiver jmsMessageReceiver) {
